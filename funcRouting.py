...
def __through_allOutlets_noDIV(outletINFO, Qrouted, catchLoc, t_lag):
    """ONLY WHEN PIXELS DO NOT HAVE A UNIFORM SIZE"""
    # Dictionary to keep track of how many times water was added to each outlet
    count = {x: len(catchLoc[x]) for x in Qrouted}
...
def _multiOutlet_routing_noDIV(self, Qsim):
    """ONLY WHEN PIXELS DO NOT HAVE A UNIFORM SIZE"""
    ...
    Qmulti = __through_allOutlets_noDIV(outletINFO=self.outletInfo,
                                Qrouted=Qrouted,
                                catchLoc=self.catchLoc,
                                t_lag=self.tau)
